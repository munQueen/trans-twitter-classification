...
# two different possible ways to classify:
...
print("Value counts, with binary categories: \n", full_data.binary_category.value_counts())
print("Value counts, with ternary categories: \n", full_data.ternary_category.value_counts())
...
# find tweets which are outliers in VADER and VADER misclassifies 
...
def process_quotes(text):
   ...
   #even number of quotes - find each quoted section, then put each word in those sections in individual quotes
   else:
...
# process the text, using our function to keep all quotes  
...
# also want to remove all quotes for comparison
...
# for all models : data will be upsampled so that we have balanced classes and use all data 
X_train, X_test = train_test_split(full_data[['compound', 'binary_category']],  test_size=0.4, random_state=42)
...
tf_scores_noquotes = tfidf_noquotes.fit_transform(full_data.cleaned_text_noquotes)
...
#upsample the data to have balanced classes
upsample_amount = X_train.binary_category.value_counts()['supportive'] - X_train.binary_category.value_counts()['negative'] 
upsample_data = X_train[X_train.binary_category == 'negative'].sample(n=upsample_amount, random_state=42, replace=True)
X_train = X_train.append(upsample_data)
y_train = X_train.binary_category
...
#print out some evaluation tools 
...
#create a matrix of each of the features from the vectorizer
words = np.array(tfidf_noquotes.get_feature_names())
x = sparse.eye(X_test.shape[1])
#find the predicted probability for each of these words
...
#TFIDF - repeat the process, this time use the data which still has quoted words
...
#take this model and save it so that it can be used in other programs
...
#two part model with the vader scores - this wasn't as successful
...
#code to be able to manually review false positives and false negatives for finding trends
